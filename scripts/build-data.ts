 ...
 * Scans a Notion export folder, parses CSVs + MDs, and emits static JSON.
 *
 * Usage: npm run build:data
 * Expects data/meetings in the project root.
 */
...
import { extractUid, extractTitle } from './lib/uid-extractor.js';
...
import type { DatabaseDef, PageDef, ScannedFile } from './lib/types.js';

const PROJECT_ROOT = path.resolve(import.meta.dirname, '..');
const EXPORT_DIR = path.join(PROJECT_ROOT, 'data', 'meetings');
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'public', 'data');

console.log('Scanning export directory:', EXPORT_DIR);

// 1. Scan all files
const allFiles = scanDirectory(EXPORT_DIR);
...
for (const mf of mdFiles) {
  ...
  // e.g., page at "Private & Shared/Amgen Task Board/foo.md" → DB "Amgen Task Board"
  let databaseUid: string | undefined;
  let bestMatchDepth = -1;
  for (const db of databases) {
    // Check if any ancestor directory matches this DB's title
    for (let d = 0; d < mf.dirParts.length; d++) {
      if (normalizeTitle(mf.dirParts[d]) === normalizeTitle(db.title)) {
        // Deeper match = more specific = better
        if (d > bestMatchDepth) {
          bestMatchDepth = d;
          databaseUid = db.uid;
        }
  ...
  // Asset folder name = page title (with URL encoding from Notion)
  const titleForDir = extractTitle(mf.name);
  const parentDir = mf.dirParts.join('/');
  // The asset directory would be: parentDir/titleForDir
...
// 7. Emit all JSON
emitJson(OUTPUT_DIR, databases, pages, 'meetings');
